...
figure1_A <- paste(figure1_idbanks_A, collapse = "+") |>
  ...
  ggplot() + ylab("Weight, 06 - Health") + xlab("") + theme_minimal() +
  geom_line(aes(x = date, y = OBS_VALUE/10000, color = `CPI or HICP?`)) +
...
figure1_B <- paste(figure1_idbanks_B, collapse = "+") |>
  ...
  transmute(date = as.Date(paste0(TIME_PERIOD, "-01")),
            ...
            `CPI or HICP?` = case_when(grepl("harmonisé", TITLE_FR) ~ "HICP",
                                       T ~ "CPI")) %>%
  group_by(`CPI or HICP?`) %>%
  filter(date >= as.Date("1996-01-01"),
         date <= as.Date("2024-06-01")) %>%
  arrange(date) %>%
  mutate(OBS_VALUE = 100*OBS_VALUE/OBS_VALUE[1]) %>%
  ggplot() + ylab("Price Index - Health (100 = 1996)") + xlab("") + theme_minimal() +
  geom_line(aes(x = date, y = OBS_VALUE, color = `CPI or HICP?`)) +
  scale_x_date(breaks = seq(1920, 2100, 2) %>% paste0("-01-01") %>% as.Date,
               labels = date_format("%Y")) +
  theme(legend.position = c(0.7, 0.55),
        legend.title = element_blank(),
